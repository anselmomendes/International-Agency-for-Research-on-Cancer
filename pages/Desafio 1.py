...
attributable['Attributable fraction M'] = attributable['paf_obs_m'].apply(lambda x: '0%' if pd.isnull(x) else str(round(x,2)*100) + '%')
attributable['Attributable fraction F'] = attributable['paf_obs_f'].apply(lambda x: '0%' if pd.isnull(x) else str(round(x,2)*100) + '%')
attributable['Attributable fraction B'] = attributable['paf_obs_both'].apply(lambda x: '0%' if pd.isnull(x) else str(round(x,2)*100) + '%')
